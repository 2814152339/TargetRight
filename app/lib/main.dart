...
class _DynamicIslandDripPageState extends State<DynamicIslandDripPage>
  ...
  void initState() {
    ...
    _onboardingCardsEntryController = AnimationController(
      vsync: this,
    ...
    _reminderScheduler = _InAppReminderScheduler(
      onTrigger: _handleReminderDueAlarmFlow,
    );
  ...
  Future<void> _runOnboardingFlow() async {
    ...
    await _onboardingCardsEntryController.forward(from: 0);
  ...
  Future<void> _showOverlayPrompt(
    ...
    if (secondaryText != null) {
      ...
      if (!mounted || version != _onboardingOverlayVersion) {
        return;
      }
      setState(() {
  ...
  Future<void> _fadeOutOverlayPrompt({int? expectedVersion}) async {
    ...
    if (expectedVersion != null && expectedVersion != _onboardingOverlayVersion) {
      return;
    }
  ...
  void _handleOnboardingPanelControllerChanged() {
    ...
    if (_onboardingStage == _OnboardingStage.waitingRightSwipe) {
      if (_panelController.value > 0.04 &&
        ...
        unawaited(
          _fadeOutOverlayPrompt(expectedVersion: _onboardingOverlayVersion),
        );
      }
  ...
  Future<void> _consumePendingNativeReward() async {
    ...
    try {
      ...
      // Keep the app usable even when the native bridge is unavailable.
    }
  }
  ...
  Future<void> _handleReminderDueAlarmFlow(_ReminderTaskConfig task) async {
    if (!mounted || _alarmDialogVisible) {
      return;
    }
    _alarmDialogVisible = true;
                          ...
                          builder: (context, _) {
                            ...
                            return Stack(
                              children: <Widget>[
                                if (widget.showCards)
                                  Positioned(
                                    left: media.width * 0.050 + contentEntryOffset,
